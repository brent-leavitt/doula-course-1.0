...
/* 	private static $log_dir_path = '';
	...
	public function load_email_student_editor(){
		...
		if ( isset( $_POST ) && !empty( $_POST['humming-birds-and-bees'] ) ) {
			...
			switch( $sPostValue ){
				case 'subscrip_cancel_upcoming':
					$subject = 'Student Account Cancellation Pending';
					$body = <<<EOT
Greetings $first_name,

We have received notice that your payment subscription has been canceled and no further payments will be collected or credited to your student account. 

Your student account will remain active until the end of your current billing period (one month after your last payment was received). If you're wanting to cancel your enrollment with New Beginnings Doula Training, no further action is required. 

----

If you would like to pause with your training and resume at a later date, please send me a note to resume training when are ready to return. 
EOT;
					$admin_notes = 'Payment subcription has been canceled. Inactive account status pending. Notice Sent.';
					
					break;
					
				case 'subscrip_cancel_expired':
					$subject = 'Student Account Canceled';
					$body = <<<EOT
Greetings $first_name,

We have received notice that your payment subscription has been canceled and no further payments will be collected or credited to your student account. 

Your student account has been moved to inactive status. If you're wanting to cancel your enrollment with New Beginnings Doula Training, no further action is required. 

----

If you would like to pause with your training and resume at a later date, please send me a note to resume training when are ready to return. 
EOT;
					$admin_notes = 'Payment subcription has been canceled. Account moved to inactive status. Notice Sent.';
					$admin_actions = array(
						'role' =>'student_full_inactive'
					);
					break;
					
				case 'subscrip_cancel_dayof':
					$subject = 'Student Account Canceled';
					$body = <<<EOT
Greetings $first_name,

Your payment subscription has been canceled and no further payments are being collected or credited to your student account. 

Your student account is now moved to inactive status. If you're wanting to cancel your enrollment with New Beginnings Doula Training, no further action is required. 

----

If you would like to pause with your training and resume at a later date, please send me a note to resume training when are ready to return. 
EOT;
					$admin_notes = 'Payment subcription is canceled. Account moved to inactive status. Notice Sent.';
					$admin_actions = array(
						'role' =>'student_full_inactive'
					);
					break;
					
				case 'subscrip_suspend':
					$subject = 'Payment Subscription Suspended';
					$body = <<<EOT
Greetings $first_name,

We received notice that your payment subscription has been suspended and no further payments will be collected or credited to your student account. This is typically caused by one of two reasons: an expired credit card number or insufficient funds in your payment account.

If you would like to continue with your training, you will need to establish a new payment subscription. Please contact me for details. 

If you're wanting to cancel your enrollment with New Beginnings Doula Training, no further action is required and your student account will be deactivated. 				
EOT;
					$admin_notes = 'Payment subscription has been suspended. Student account moved to inactive status. Notice Sent. ';
					$admin_actions = array(
						'role' =>'student_full_inactive'
					);
					break;
...
EOT;
				...
				case 'almn_renewal':
					$subject = 'An Invitation to Renew';
					$body = <<<EOT
Greetings  $first_name,

First off, on behalf of the New Beginnings team, let me just take a moment to say "Thank you!" in a very big way. Without your commitment to our training program, we would not be here today, still building and improving birth outcomes for mothers and babies. This past year (despite and maybe because of a global pandemic) has been one of our most exciting years yet. Here's just a few highlights: 

* Hospital restrictions as a result of pandemic lock-downs launched doulas into a new light because they were allowed into the birthing space when many others were not.
* With many in-person training options put on-hold, online doula training became the primary form of training available during this period of increased awareness.
* State governments have en-mass begun adding doula work to insurable birth-related services in response to the maternal mortality rate increases in the United States.

However you slice it, doula work has increased in awareness over the past year for its natural benefits to the birthing process, which are now backed by evidence-based research. 

---

Your alumni status with New Beginnings has past its two year mark and as such is currently inactive. We would be honored to continue to recognize your certification. To renew, simply reply to request renewal instructions. You can also use the following link: https://wordpress-646539-2107765.cloudwaysapps.com/billing/renew-certification/
EOT;
...
EOT;
		...
		$selectArr = array(
			'ISSUES' => array(
				'subscrip_cancel_upcoming'=>'Subscription Canceled (Upcoming)',
				'subscrip_cancel_expired'=>'Subscription Canceled (Expired)',
				'subscrip_cancel_dayof'=>'Subscription Canceled (Day Of)',
				'subscrip_suspend'=>'Subscription Suspended'
			),
			'MOTIVATION' => array(
...
-----------------------------------------------------  ";
		...
		$nb_trans = new nb_transaction($emlWho);
		...
		$emlWhy = 'new_student';
		$emailed = $nb_trans->email_receipt($emlWho, $emlWhat, $emlWhy);
